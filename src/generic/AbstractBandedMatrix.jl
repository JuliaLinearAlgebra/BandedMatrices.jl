...
```
"""
@inline colstart(A, i::Integer) = max(i-bandwidth(A,2),  firstindex(A,1))
...
```
"""
@inline colstop(A, i::Integer) = min(i+bandwidth(A,1), lastindex(A,1))
...
```
"""
@inline rowstart(A, i::Integer) = max(i-bandwidth(A,1), firstindex(A,2))
...
```
"""
@inline rowstop(A, i::Integer) = min(i+bandwidth(A,2), lastindex(A,2))
