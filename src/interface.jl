...
banded_axpy!{U, V}(a::Number, X::AbstractMatrix{U}, Y::AbstractMatrix{V}) = banded_generic_axpy!(a, X, Y)

# matrix * vector

# for AbstractMatrix, uses the generic version of multiplication
banded_matvecmul!{T, U, V}(c::AbstractVector{T}, tA::Char, A::AbstractMatrix{U}, b::AbstractVector{V}) = banded_generic_matvecmul!(c, tA, A, b)
...
banded_matmatmul!{T, U, V}(C::AbstractMatrix{T}, tA::Char, tB::Char, A::AbstractMatrix{U}, B::AbstractMatrix{V}) = banded_generic_matmatmul!(C, tA, tB, A, B)
